...
export const CardPlaces = () => {
  ...
  const handleTransitionEnd = () => {
    if (!firstCardRef.current || !carouselRef.current) return;
    ...
    if (carouselRef.current.scrollLeft === 0) {
      ...
      carouselRef.current.scrollLeft =
        carouselRef.current.scrollWidth / 2 - cardWidth;
    }
  };
        ...
        {store.places.map((place, index) => (
          <li className="card-c" key={index} ref={firstCardRef}>
            ...
            <div className="card-c-footer">
              <button
                className="button"
                onClick={() => handleLearnMore(place.id)}
              >
                {" "}
                Saber más{" "}
              </button>
